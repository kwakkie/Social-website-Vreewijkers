...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
    ...
    // Get the avatar from the author 
	...
	$sql = "INSERT INTO status(account_name, author, avatar, type, data, postdate) 
			VALUES('$account_name','$log_username','$avatar','$type','$data',now())";
	$query = mysql_query($sql);
	$id = mysql_insert_id();
	mysql_query("UPDATE status SET osid='$id' WHERE id='$id' LIMIT 1");
	...
	if ($row[0] > 9) { // If they have 10 or more posts of type a
		...
		$row = mysql_fetch_row($query);
		$oldest = $row[0];
		mysql_query("DELETE FROM status WHERE osid='$oldest'");
	}
	// Insert notifications to all friends of the post author
	$friends = array();
	$query = mysql_query("SELECT user1 FROM friends WHERE user2='$log_username' AND accepted='1'");
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) { array_push($friends, $row["user1"]); }
	$query = mysql_query("SELECT user2 FROM friends WHERE user1='$log_username' AND accepted='1'");
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) { array_push($friends, $row["user2"]); }
	for($i = 0; $i < count($friends); $i++){
		$friend = $friends[$i];
		$app = "Status Post";
		$note = $log_username.' heeft een bericht geplaatst op: <br /><a href="user.php?u='
                .$account_name.'#status_'
                .$id.'">'
                .$account_name.'&#39;s Profile</a>';
		mysql_query("INSERT INTO notifications(username, initiator, app, note, date_time) VALUES('$friend','$log_username','$app','$note',now())");			
	}
	mysql_close();
	echo "post_ok|$id";
	exit();
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	$sql = "INSERT INTO status(osid, account_name, author, avatar, type, data, postdate)
	        VALUES('$osid','$account_name','$log_username','$avatar','b','$data',now())";
	$query = mysql_query($sql);
	$id = mysql_insert_id();
	...
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
		...
		$app = "Status Reply";
		$note = $log_username.' heeft hier gereageerd:<br /><a href="user.php?u='.$account_name.'#status_'.$osid.'">Klik hier om te bekijken</a>';
		mysql_query("INSERT INTO notifications(username, initiator, app, note, date_time) 
...
if (isset($_POST['action']) && $_POST['action'] == "delete_status"){
	...
	$query = mysql_query("SELECT account_name, author FROM status WHERE id='$statusid' LIMIT 1");
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
		$account_name = $row["account_name"]; 
		$author = $row["author"];
	}
    if ($author == $log_username || $account_name == $log_username) {
		mysql_query("DELETE FROM status WHERE osid='$statusid'");
		mysql_close();
	    echo "delete_ok";
		exit();
	}
...
if (isset($_POST['action']) && $_POST['action'] == "delete_reply"){
	...
	$query = mysql_query("SELECT osid, account_name, author FROM status WHERE id='$replyid' LIMIT 1");
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
		$osid = $row["osid"];
		$account_name = $row["account_name"];
		$author = $row["author"];
