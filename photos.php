...
if($u == $log_username && $user_ok == true){
	...
	$photo_form .=   '<h3>Hi '.$u.', voeg een nieuwe foto toe</h3>';
	$photo_form .=   '<b>Kies album:</b> ';
	$photo_form .=   '<select name="gallery" required>';
	$photo_form .=     '<option value=""></option>';
	$photo_form .=     '<option value="Myself">Persoonlijk</option>';
	$photo_form .=     '<option value="Family">Familie</option>';
	$photo_form .=     '<option value="Pets">Huisdieren</option>';
	$photo_form .=     '<option value="Friends">Vrienden</option>';
	$photo_form .=     '<option value="Random">Random</option>';
	$photo_form .=   '</select>';
	$photo_form .=   ' &nbsp; &nbsp; &nbsp; <b>Kies foto:</b> ';
	$photo_form .=   '<input type="file" name="photo" accept="image/*" required>';
	$photo_form .=   '<p><input type="submit" value="Upload foto"></p>';
	$photo_form .= '</form>';
}
...
if(mysql_num_rows($query) < 1){
	$gallery_list = "Deze gebruiker heeft nog geen foto's.";
} else {
	while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
		$gallery = $row["gallery"];
		$countquery = mysql_query("SELECT COUNT(id) FROM photos WHERE user='$u' AND gallery='$gallery'");
		$countrow = mysql_fetch_row($countquery);
		$count = $countrow[0];
		$filequery = mysql_query("SELECT filename FROM photos WHERE user='$u' AND gallery='$gallery' ORDER BY RAND() LIMIT 1");
		$filerow = mysql_fetch_row($filequery);
		$file = $filerow[0];
		$gallery_list .= '<div>';
...
function showGallery(gallery,user){
	...
	_("section_title").innerHTML = user+'&#39;s '+gallery+' Gallery &nbsp; <button onclick="backToGalleries()">Terug naar albums</button>';
	_("photos").style.display = "block";
	_("photos").innerHTML = 'loading fotos ...';
...
function photoShowcase(picdata){
	...
	if("<?php echo $isOwner ?>" == "yes"){
		_("picbox").innerHTML += '<p id="deletelink"><a href="#" onclick="return false;" onmousedown="deletePhoto(\''+data[0]+'\')">Verwijder deze foto <?php echo $u; ?></a></p>';
	}
}
function closePhoto(){
	_("picbox").innerHTML = '';
...
function deletePhoto(id){
	var conf = confirm("OK om deze foto definitief te verwijderen.");
	if(conf != true){
		return false;
	}
...
<div id="pageMiddle">
  ...
  <h2 id="section_title"><?php echo $u; ?>&#39;s Foto albums</h2>
  <div id="galleries"><?php echo $gallery_list; ?></div>
  <div id="photos"></div>
  <div id="picbox"></div>
  <p style="clear:left;">Deze fotos zijn van <a href="user.php?u=<?php echo $u; ?>"><?php echo $u; ?></a></p>
</div>
